    ...
    case 0x1f:
      ...
      this.cpu.gprs[0] =
        (key /
          Math.pow(
            2,
            (180 - this.cpu.gprs[1] - this.cpu.gprs[2] / 256) / 12
          )) >>>
        0;
      break;
    default:
...
GameBoyAdvanceInterruptHandler.prototype.setInterruptsEnabled = function (
  value
) {
...
GameBoyAdvanceInterruptHandler.prototype.dmaSetSourceAddress = function (
  dma,
  address
) {
...
GameBoyAdvanceInterruptHandler.prototype.dmaSetDestAddress = function (
  dma,
  address
) {
...
GameBoyAdvanceInterruptHandler.prototype.dmaSetWordCount = function (
  dma,
  count
) {
...
GameBoyAdvanceInterruptHandler.prototype.dmaWriteControl = function (
  dma,
  control
) {
...
GameBoyAdvanceInterruptHandler.prototype.timerSetReload = function (
  timer,
  reload
) {
...
GameBoyAdvanceInterruptHandler.prototype.timerWriteControl = function (
  timer,
  control
) {
...
GameBoyAdvanceInterruptHandler.prototype.lz77 = function (
  source,
  dest,
  unitsize
) {
  ...
  while (remaining > 0) {
    if (blocksRemaining) {
      if (blockheader & 0x80) {
        ...
        sPointer += 2;
        disp =
          dPointer - (((block & 0x000f) << 8) | ((block & 0xff00) >> 8)) - 1;
...
GameBoyAdvanceInterruptHandler.prototype.rl = function (
  source,
  dest,
  unitsize
) {
